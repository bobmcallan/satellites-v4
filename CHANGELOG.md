...
- **Hot-runner Go surface — `dispatch_class` retired; bash recipes live in `type=skill`** (sty_447b9fe0, `epic:role-boundaries`) — the worker no longer inspects a contract's `dispatch_class` frontmatter; every dispatched task runs the seven-step heavy claude subprocess. Deleted `internal/agent/worker/hotpath.go` (1142 LOC: `runHotPath`, `runCommitHotPath`, `runMergeToMainHotPath`, `appendHotCommitEvidence`, `appendHotMergeReleaseEvidence`/`…pushedUnverified`/`…idempotent`, `pollPprodConverge`, `fetchPprodCommit`, `parseGHRunID`, `verifyChainPriorWorkSuccess`, `buildPriorIndex`, `hasSuccessorRetry`, `closeTaskSuccess`/`closeTaskFailure`, `appendEvidence`, `failMergePrePush`/`failMergePostMergePrePush`/`failMergePostPush`, the `effective*Timeout` / `effective*Successes` helpers, and the package-level `runGH` indirection), `internal/agent/worker/hotpath_test.go` (1416 LOC, co-deleted), and `internal/agent/worker/chain_shape_test.go` (covered the deleted `verifyChainPriorWorkSuccess` helper; the cross-cutting assertion survives in `tests/integration/auto_supersession_test.go`). Removed the `dispatch_class` parse path in `internal/configseed/parsers.go::contractToInput` plus the `dispatch_class:` frontmatter line from every system + workspace contract seed (`commit.md`, `merge_to_main.md`, `develop.md`, `review.md`, `plan.md`, `story_close.md`, `story_review.md`, `substrate_audit.md`). Removed the `contractInfo.Structured` field + `contractInfo.dispatchClass()` method + the hot-path selector block in `internal/agent/worker/client_claude.go::Execute`. The new workspace-tier `skill:git_commit` + `skill:git_merge_to_main` documents under `config/seed/wksp_5b3257d1/skills/` carry the bash recipes the releaser_agent now invokes via the Bash tool + observes the literal stdout against documented success/failure shapes (the `pr_substrate_model` configuration-over-code shape — restoring symmetry: behaviour was already "no LLM in Go" for develop/plan/review; the hot-path was the asymmetry). The releaser_agent doc gains `skill_refs: [git_commit, git_merge_to_main]` + an "Invoke + observe pattern" section citing the `ldg_a4e759a9` false-success incident. Architectural regression test `internal/agent/worker/dispatch_action_agnostic_test.go::TestDispatcher_NoPerActionBranching` greps the dispatch path source text and fails on any re-introduction of `ti.Action == "contract:commit"`-shape branches, `switch ti.Action` / `switch task.Action`, `dispatchClass`, or `runHotPath`. The `developActionEpilogue` PROMPT-string shaper is allow-listed (it shapes the prompt the dispatched claude reads; it is not a runtime dispatch branch). Per `## Version bump policy`: bumps `[satellites-server]`, `[satellites-client]`, AND `[satellites-agent]` — the deletion touches code consumed by every binary's build graph.

- **`[client/agent].toml` `converge_request_timeout` + `converge_consecutive_successes` keys — silent removal** (sty_447b9fe0) — the per-request HTTP timeout and N-of-M-consecutive-success knobs were consumed only by the deleted `pollPprodConverge` loop in `hotpath.go`. The `Config.ConvergeRequestTimeout` / `Config.ConvergeConsecutiveSuccesses` fields are removed from both `internal/cliconfig/cliconfig.go` and `internal/config/agent_config.go`; the corresponding TOML overlay entries (`converge_request_timeout`, `converge_consecutive_successes`) are no longer parsed. Operators carrying these keys in `bin/satellites-client.toml` or `bin/satellites-agent.toml` will see a TOML unknown-field warning on next boot — remove the keys at swap time.

### Added

- **`workspace_delete` + `project_delete hard=true` + `project_move_workspace` MCP verbs — substrate cleanup primitives** (sty_d357b28d, `epic:substrate-cleanup`) — three new operator-facing destructive ops fill the gap that previously left workspaces unremovable, archived projects un-purgeable, and projects locked to their original workspace. `workspace_delete(id)` hard-removes the workspace row + every member row; refuses with `workspace_has_projects` when any project (active OR archived) still carries the workspace_id, and `not_admin` when the caller is not a workspace admin. `project_delete` gains a `hard: bool` param: default behaviour (soft-archive cascade) is preserved byte-for-byte; `hard=true` requires `status=archived` first (refuses `project_not_archived` otherwise) and cascade-purges every dependent row across stories, tasks, ledger, agent api-keys, and repo rows before removing the project row itself. `project_move_workspace(id, target_workspace_id)` cascades a project's `workspace_id` to a new target plus the same rewrite on every dependent table; admin of BOTH source AND target workspaces required (`not_admin_source` / `not_admin_target`); idempotent when target == current. Each verb returns per-table cascade counters so the operator can audit the destructive op from the response envelope alone. Store-level surface: `project.Store.{Delete, ListByWorkspaceID}`, `workspace.Store.Delete`, plus `DeleteByProjectID` + `SetWorkspaceIDByProjectID` on `story.Store`, `task.Store`, `ledger.Store`, `auth.APIKeyStore`, `repo.Store` (both Memory and Surreal impls). The append-only ledger posture is preserved by default — the hard-delete path is the explicit operator-driven escape valve, gated on `hard=true` and the prior soft-archive step. Per `## Version bump policy`: bumps `[satellites-server]` only (touches `internal/{client,mcpserver,project,workspace,story,task,ledger,auth,repo}/` — all consumed by `cmd/satellites-server/`). Regression tests: `internal/mcpserver/substrate_cleanup_handlers_test.go` (8 tests covering refuse-on-non-empty workspace + happy-path empty + non-admin reject; refuse-on-non-archived project + cascade-after-soft-archive; idempotent same-source-target + cascade-workspace-id + non-admin-target reject); `internal/ledger/ledger_test.go::TestStoreInterface_Surface` updated to reflect the new `DeleteByProjectID` + `SetWorkspaceIDByProjectID` interface methods.

- **`satellites-client task render-prompt` + `task_render_prompt` MCP verb — orchestrator-side inline prompt builder** (sty_72e36256, `epic:role-boundaries`) — adds a self-contained markdown renderer the orchestrator pipes into `task_add(prompt=…)` so dispatched executors running under the restricted role envelope (`sty_056b68f6`) carry their full context — agent body, contract body, agent.skill_refs ∪ contract.skills_required, cited principles (regex `pr_[a-z0-9_]+` over contract + story prose), prior task chain — without invoking `story_get` / `agent_get` / `contract_get` / `principle_list` / `task_walk` (those verbs all 403 under the task-scoped apikey). The library lives in `internal/client/render_prompt.go::RenderTaskPrompt`; the CLI shim is `cmd/satellites-client/task_render_prompt.go::newTaskRenderPromptCmd` (74 lines, writes raw markdown to stdout — no JSON envelope, no header assembly). Per-section byte caps (`AgentCap=4096`, `ContractCap=4096`, `SkillCap=4096`, `StoryCap=8192`, `PrincipleCap=2048`) backed off to the previous UTF-8 codepoint boundary, with a literal marker `[…cited body continues; see document_get(id=<doc_id>) …]` appended on truncation. The orchestrator agent doc (`config/seed/system/agents/claude_orchestrator.md`) plan-submission section gains a new step 3 "Render the prompt" naming `task render-prompt` explicitly per `pr_substrate_model` — context now flows in-prompt, not via runtime read verbs. Per `## Version bump policy`: bumps `[satellites-client]` (`cmd/satellites-client/` + `internal/client/` consumed by it). Regression tests: `internal/client/render_prompt_test.go` (all-sections / omits-empty / idempotent / cap-verbatim / cap+1 / UTF-8 boundary / skill-union / cited-principles).

- **`default_lifecycle` workflow doc + `task_walk.lifecycle_status` advisory drift signal** (sty_e0c3d615) — the dispatch lifecycle `plan → (develop → review → iterate)+ → commit → push → close` is now a workspace-scope workflow document at `config/seed/wksp_5b3257d1/workflows/default_lifecycle.md` (the system-tier `default` workflow stays as the canonical pattern). `TaskWalkOutput.LifecycleStatus` carries the chain-shape verdict computed by `internal/client/lifecycle.go::computeLifecycleStatus`: `on_shape` or `drifted:<reason>` (`plan_absent` | `review_skipped` | `close_before_push` | `phase_unknown:<action>`). `story_close` (§6) appends one `kind:lifecycle-drift` ledger row tagged `reason:<drift_reason>` on a drifted chain, idempotent per reason — the gate does NOT refuse to close on drift (initially advisory). The workflow doc body registers the `pr_lifecycle_shape` citation slot reviewers MAY cite when the gap is reviewer-relevant; authoring the principle itself is a follow-up. `claude_orchestrator.md`, `plan.md`, `develop.md`, and `story_close.md` carry lifecycle-reference subsections pointing at `default_lifecycle`. Per `## Version bump policy`: bumps `[satellites-server]` only (`internal/client/`, `config/seed/` consumed by `cmd/satellites-server/`). Regression tests: `internal/client/lifecycle_test.go::TestComputeLifecycleStatus` (table-driven across every drift reason + `on_shape`), `internal/client/story_close_test.go::TestStoryClose_LifecycleDriftRowAppended` / `_LifecycleDriftRowIdempotent` / `_LifecycleOnShapeNoDriftRow`, `internal/client/task_test.go::TestTaskWalk_LifecycleStatusOnEmptyChain` / `_LifecycleStatusOnShape`, and `internal/configseed/scope_precedence_test.go::TestScopePrecedence_WorkspaceLifecycleWorkflowLoads`.

### Fixed

- **Portal `activeWorkspace` fallback no longer fakes single-workspace identity; case-insensitive workspace name collisions disambiguated** (sty_a21f61a6) — when a user with multiple workspaces had no sticky `session.ActiveWorkspaceID` (or the saved id no longer matched a current membership), `internal/portal/portal.go::activeWorkspace` returned `chips[0]` as the visible "active" while keeping the data scope wide. The nav header rendered `WORKSPACE / <chips[0].Name>`, the dropdown template hid whichever chip matched the active id, and `/projects` queried `ListByOwner(user.ID, fullIDs)` so rows from every workspace appeared simultaneously — the header lied about a single-workspace identity while the data path was multi-workspace. The fallback now branches: single membership stays auto-active (no ambiguity); multi-membership with no/stale sticky returns an empty `wsChip{}` active so the header renders the existing em-dash placeholder, the dropdown shows every chip (the empty id matches no chip in the template's `{{if ne .ID $activeID}}` filter), and the scope still spans every membership. Adds `disambiguateChipNames` so two workspaces whose trimmed display names match case-insensitively (e.g. `personal` / `Personal`) render with `(wksp_<id>)` appended on every chip surface. Per `## Version bump policy`: bumps `[satellites-server]` only (`internal/portal/` consumed by `cmd/satellites-server/`). Regression tests: `internal/portal/active_workspace_test.go` (`TestDisambiguateChipNames_CaseCollisionAppendsID` / `_NoCollisionPassthrough` / `_SingleChipPassthrough` / `_TripleCollisionAllSuffixed`, `TestNav_NoStickyMultiMembership_HeaderEmDashAndAllChipsInDropdown`, `TestNav_NoStickySingleMembership_HeaderShowsThatWorkspace`, `TestNav_StaleStickyMultiMembership_FallsBackToEmDash`, `TestProjectsList_NoStickyMultiMembership_SpansAllWorkspaces`, `TestProjectsList_StickyMultiMembership_NarrowsToActiveWorkspace`, `TestNav_CaseCollisionDisambiguates`).

- **`merge_to_main` hot-path treats already-merged branches as idempotent success** (sty_ca6299c9) — when a retry runs on a work branch whose tip is already reachable from `main` (a prior iter completed the merge and main has since advanced past it), the existing ff-only ancestor check `git merge-base --is-ancestor main <branch>` exits 1 and the runner failed with `merge_to_main: ff-only ancestor check: ... exit status 1`, permanently stranding any chain that overlapped with another story's shipping window (the dogfood failure on `task_61bcfde7` against sty_224774f0's source tip `983a544a`). A new preflight `git merge-base --is-ancestor <branch> main` runs immediately before the existing ff-only check; on exit 0 the runner resolves the source tip via `git rev-parse <branch>`, appends a `kind:release-evidence` row tagged `release:idempotent` carrying `pushed_sha:<sourceTip>`, and closes success — no merge, no push, no `gh run watch`, no pprod converge poll. The historical release row for that SHA carries the converge proof; sty_1478a1df's ancestry-aware deploy:behind gate locates it on the standard release-evidence walk. Per `## Version bump policy`: bumps `[satellites-client]` only (`internal/agent/worker/` is consumed by `cmd/satellites-client/`). New regression test `internal/agent/worker/hotpath_test.go::TestRunMergeToMainHotPath_IdempotentAncestor` asserts the no-mutation invariant + tagged row + closed-success persistence. `seedMergeToMainHappyStubs` registers `gitErrors` for the new preflight key so the default stub behaviour (empty-output/nil-error → looks like exit 0) does not erroneously fire the idempotent path on the existing happy-path tests.

- **`story_close` deploy:behind gate is now ancestry-aware via release-evidence ledger walk** (sty_1478a1df) — the gate previously compared pprod's reported commit against the story's `pushed_sha` via prefix-match (sty_1e2f7ae7). Under trunk-based ff-only releases, this rejected any story whose work had already shipped past a child story's release — `ae12fe7…` IS in pprod's history when pprod reports `95d6047…` (parent → child), but prefix-match said no. The gate now walks project-scoped `kind:release-evidence` rows newest-first to find `pprodTipRow` (the most recent CONVERGED row whose `pushed_sha` prefix-matches pprodCommit), and accepts iff `storyReleaseRow.CreatedAt <= pprodTipRow.CreatedAt`. `release:pushed_unverified` rows are excluded from tip candidacy. The existing sty_1e2f7ae7 prefix-match fast path is preserved (no walk when pprodCommit prefix-matches the story's release SHA). The sty_224774f0 `pprod_commit` / `skip_deploy_check` / `release-evidence:no-deploy-endpoint` resolver branches are unchanged. Per `## Version bump policy`: bumps `[satellites-server]` only (`internal/client/` consumed by `cmd/satellites-server/`). New regression tests in `internal/client/story_close_test.go`: `TestStoryClose_DeployBehind_AncestorAccepted` (older story row + newer tip row → pass via walk), `TestStoryClose_DeployBehind_NonAncestorRejected` (pprod commit on no row → fail), `TestStoryClose_DeployBehind_UnverifiedSuppressed` (unverified row not a tip → fail), `TestStoryClose_DeployBehind_PprodOlderThanRelease` (story newer than tip → fail with "newer than pprod tip" detail).
