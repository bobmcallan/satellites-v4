	...
	cases := []struct {
	...
	}{
		{
			...
			want: []string{
				...
				`agent_get(name="developer_agent")`,
				`contract_get(name="develop")`,
				`story_get(id="sty_aaa")`,
				`task_walk(story_id="sty_aaa")`,
				`principle_list(active_only=true, project_id="proj_x")`,
				`task_update(id="task_001", status="closed", outcome="success" | "failure")`,
				"/tmp/home_xxx",
		...
		},
		{
			...
			want: []string{
				`task_get(id="task_p")`,
				`agent_get(name="planner_agent")`,
				`contract_get(name="plan")`,
				`story_get(id="sty_p")`,
