...
// backstops document_ingest_file / document_get callers that don't supply
// their own project_id.
func SeedDefault(ctx context.Context, store Store, logger arbor.ILogger) (string, error) {
	...
	p, err := store.Create(ctx, DefaultOwnerUserID, DefaultProjectName, time.Now().UTC())
	if err != nil {
		return "", err
	}
	logger.Info().Str("project_id", p.ID).Msg("default project seeded")
	return p.ID, nil
}
